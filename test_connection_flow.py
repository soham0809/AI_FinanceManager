#!/usr/bin/env python3
"""
Test script to verify backend-frontend connection and analytics flow
"""
import requests
import json
import time
...
def test_sms_parsing():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"[FAIL] SMS Parsing Failed: {response.status_code} - {response.text}")
            return False
    except Exception as e:
...
def test_analytics_update_flow():
    ...
    try:
        ...
        if parse_response.status_code == 200:
            print(f"   ✅ Added new transaction: Netflix Rs.299")
            ...
            if updated_spending > initial_spending:
                print(f"   ✅ Analytics updated correctly! Increase: Rs.{updated_spending - initial_spending:.2f}")
                return True
            else:
                print(f"   ❌ Analytics not updated properly")
                return False
        else:
            print(f"   ❌ Failed to add transaction: {parse_response.status_code}")
            return False
            
    except Exception as e:
        print(f"   ❌ Analytics update test failed: {e}")
        return False
